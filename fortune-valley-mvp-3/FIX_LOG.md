...
- **2026-04-19** — No onboarding / guided tutorial. Players drop into Homebase and see income numbers ticking, popups firing, and three financial panels with no explanation of what the game is asking them to do. Need a Clash-of-Clans-style first-run flow where a narrator (coach character, text bubble, or step overlay) walks the player through: (1) the core objective of the game (grow the chain, beat the rival, avoid bankruptcy), (2) the three balances on the HUD (checking, investing, credit) and what each means, (3) the three financial systems (credit card, loans, insurance) and when to open each panel, (4) the income rate indicators (player vs rival), (5) the QuestionMaster and how answering questions earns rewards + reinforces learning, (6) lot purchase + restaurant upgrade flow. Should be gated on a first-play flag so returning players can skip, and each step should highlight the relevant UI element with a spotlight/arrow. Reuse `CoachChatUI`/`CoachConfig` infrastructure if it fits.
- **2026-04-19** — Camera has no zoom or pan bounds. Players can zoom out far enough to see past the play area and click-drag the camera target well outside the city, which breaks immersion and can strand the view off-scene. Add (a) min/max zoom clamps on the camera rig, and (b) a pan/drag fence that clamps the camera focus point to a bounding box covering the 7 lots plus a small margin. Values should be SerializeField on the camera controller so they are tunable in the Inspector.
